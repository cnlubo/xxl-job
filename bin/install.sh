...
export SQL_SOURCE_PATH="${DIR}/db/tables_xxl_job.sql"
...
uncompress_packages() {
  ...
  for pack in ${list}; do
    ...
    if [ ${#PACKAGE_NAMES[@]} -gt 0 ]; then
      uncompress=false
      for server in "${PACKAGE_NAMES[@]}"; do
...
scan_to_install_modules() {
  echo "Scan modules directory: [$1] to find server under xxl-job-amin"
  let c=0
  ls_out=$(ls $1)
  for dir in ${ls_out}; do
